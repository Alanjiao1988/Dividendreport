...
每份报告正文开头须有 `dividend-report-meta` 注释块，字段 `ticker`、`company`、`exchange`、`as_of_date`、`published_at`、`ruleset` 与索引一致；索引有 `supersedes` 时注释块须给出相同值，索引没有时注释块也不得出现该字段。HTML 可将注释放在 `<!DOCTYPE html>` 后；其 `<html data-ruleset="...">` 同样须与索引一致，并包含 `html`、`body` 和 UTF-8 字符集声明。缺少注释块会使全部元数据交叉核对失效，因此验证器将其视为错误而非跳过。

港股代码补至至少四位并保留有效五位代码，例如 `0836.HK`。路径为 `reports/<ticker>/<YYYY-MM-DD>-<company-slug>-<ticker>.md` 或 `.html`。新 HTML 报告保持单文件，正文、样式及图表内嵌，无需联网或脚本即可阅读；不把 HTML 改名为 Markdown 或批量转换旧报告。新文件名采用可读名称与连字符；保留已有合法路径，括号本身不是断链。移动路径时同步所有引用。
...
Markdown 摘录保持原始文本精确匹配。HTML 摘录使用静态正文文字：解析实体、忽略内联标记并归一化空白，不允许注释、属性、页头、脚本、样式、模板或显式隐藏内容充当证据。验证器不执行脚本或计算任意样式表；发布者仍需确认摘录在实际报告中可见、语义一致。不得通过添加隐藏数字使摘要过检。
...
本地生成时间与归档发布时间分开保留；归档时不更新研究基准日或投资结论。新报告可采用规则 `2.4` 并将兼容评分记为 `Not assessed`，不得用未完成的质量分项拼成总分。版本链允许 Markdown 与 HTML 相邻，验证器会检查两种格式的未索引报告。
...
HTML 在 GitHub 文件页可能显示源码，应标明下载后用浏览器打开，不声称已建立在线查看网站。本地发布不需要 GitHub Actions、GitHub Pages 或其他托管；推送只针对获授权的仓库和分支，并在推送后核对远端提交与报告。
